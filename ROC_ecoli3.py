...
def main():
    ...
    df=pd.read_csv(pathe+"file_proposed_ecoli3.csv")
    ...
    df=pd.read_csv(pathe+"file_Boderline1_ecoli3.csv")
    df.columns = [*df.columns[:-1], 'Class']
    df['Class'] = df['Class'].apply({' positive':0, ' negative':1}.get)
    df=shuffle(df)
    base_fpr, mean_tprs=ROC_graph_create(df,KNeighborsClassifier(n_neighbors=10))
    ...
    df=pd.read_csv(pathe+"file_ADASYN_ecoli3.csv")
    df.columns = [*df.columns[:-1], 'Class']
    df['Class'] = df['Class'].apply({' positive':0, ' negative':1}.get)
    df=shuffle(df)
#    print(df)
    ...
    df=pd.read_csv(pathe+"file_Safelevel_SMOTE_ecoli3.csv")
    df.columns = [*df.columns[:-1], 'Class']
    df['Class'] = df['Class'].apply({' positive':0, ' negative':1}.get)
    df=shuffle(df)
    base_fpr, mean_tprs=ROC_graph_create(df,KNeighborsClassifier(n_neighbors=10))
