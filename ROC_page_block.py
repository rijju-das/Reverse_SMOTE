...
def main():
    ...
    df=pd.read_csv(pathb+"file_proposed_page_block.csv")
    ...
    df=pd.read_csv(pathb+"file_Boderline1_page_block.csv")
    df.columns = [*df.columns[:-1], 'Class']
    df['Class'] = df['Class'].apply({' positive':0, ' negative':1}.get)
    df=shuffle(df)
    base_fpr, mean_tprs=ROC_graph_create(df,KNeighborsClassifier(n_neighbors=10))
    ...
    df=pd.read_csv(pathb+"file_ADASYN_page_block.csv")
    df.columns = [*df.columns[:-1], 'Class']
    df['Class'] = df['Class'].apply({' positive':0, ' negative':1}.get)
    df=shuffle(df)
#    print(df)
    ...
    df=pd.read_csv(pathb+"file_Safelevel_SMOTE_page_block.csv")
    df.columns = [*df.columns[:-1], 'Class']
    df['Class'] = df['Class'].apply({' positive':0, ' negative':1}.get)
    df=shuffle(df)
    base_fpr, mean_tprs=ROC_graph_create(df,KNeighborsClassifier(n_neighbors=10))
