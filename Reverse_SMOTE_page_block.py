...
def preprocess(df):
    print(df)
    df.replace(to_replace =" positive", value =1)
    df.replace(to_replace =0, value =" negative")
    print(df)
...
def main():
    ...
    path=r"C:\Users\Lenovo\NewDropbox\Dropbox\phd2019nitsilchar\code\datasets\PageBlock\\"
    df=pd.read_csv(path+"page-blocks0.dat", names= col_names)
...
#    print(df_syn)
    ...
    print("Size of minority= ",df_minority.shape)
    print("Size of majority= ", df_majority.shape)
    print("Total Size= ",df_syn.shape)
    
    df_syn=df_syn.drop(columns=['Index'])
    df_syn=preprocess(df_syn)
    df_syn.to_csv(path+"\\file_proposed_page_block.csv")
